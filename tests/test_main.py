...
def test_validate_filter_values_accepts_single_value(monkeypatch):
    ...
    dummy = pd.DataFrame({"dcid": ["x1", "x2"], "region": ["Asia", "Europe"]})
    monkeypatch.setattr(main._country_resolver, "_concordance_table", dummy)
...
def test_validate_filter_values_rejects_invalid_value(monkeypatch):
    ...
    dummy = pd.DataFrame({"dcid": ["x1"], "subregion": ["Southern Asia"]})
    monkeypatch.setattr(main._country_resolver, "_concordance_table", dummy)
    with pytest.raises(ValueError) as exc:
...
def test_get_african_countries_delegates_to_get_places(monkeypatch):
    ...
    out = main.get_african_countries(
        place_format="name_official", exclude_non_un_members=True
    )
    assert out == ["X", "Y"]
...
def test_get_african_countries_without_excluding_non_un(monkeypatch):
    ...
    called = {}
    monkeypatch.setattr(
        main, "get_places", lambda filters, place_format, raise_if_empty=False: []
    )
    # run with exclude_non_un_members=False
    main.get_african_countries(
        place_format="dcid", exclude_non_un_members=False, raise_if_empty=False
    )
...
def test_get_places_basic_filter(monkeypatch):
    ...
    result = main.get_places(
        filters={"region": "R1"}, place_format="name_official", raise_if_empty=False
    )
...
def test_get_places_empty_warns_and_returns_empty(monkeypatch, caplog):
    ...
    assert (
        "No places found for filters {'region': ['R2'], 'income_level': ['High']}"
        in caplog.text
    )
...
def test_filter_african_countries_invalid_format_raises():
    ...
    with pytest.raises(ValueError):
        main.filter_african_countries(places=["A", "B"], from_type="bad_format")
