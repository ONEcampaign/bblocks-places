...
class Disambiguator:
    ...
    def resolve(self, places: list[str], not_found: Literal["raise", "ignore"] | str = "raise", multiple: Literal["raise", "first", "ignore"] = "raise") -> dict[str, str | None]:
        """Resolves a list of places to their Data Commons IDs"""

        candidates = {}
        for chunk in split_list(places, 30):
            candidates.update(self._dc_resolver.get_candidates(chunk, place_type="Country"))
    ...
    @staticmethod
    def apply_not_found(candidates: dict, not_found: Literal["raise", "ignore"] | str = "raise"):
    ...
    @staticmethod
    def apply_multiple(candidates: dict, multiple: Literal["raise", "first", "ignore"] = "raise"):
        ...
        for place, cands in candidates.items():
            if isinstance(cands, list) and len(cands) > 1:
                if multiple == "raise":
                    ...
                    logger.warn(f"Multiple candidates found for {place}: {candidates}. Replacing with first candidate.")
...
class ConcordanceTable:
    ...
    def __init__(self):
        ...
        self._ALLOWED_TARGETS = ("dcid", "name", "iso2_code", "iso3_code", "name_short", "income_level")
    ...
    def check_allowed(self, source: str, target: str):
        ...
        if source not in self._ALLOWED_SOURCES:
            raise ValueError(f"Invalid source: {source}. Allowed sources are {self._ALLOWED_SOURCES}")
        if target not in self._ALLOWED_TARGETS:
            raise ValueError(f"Invalid target: {target}. Allowed targets are {self._ALLOWED_TARGETS}")
    ...
    def get_full_mapper(self, source, target):
        ...
        if source == target:
            return self._concordance_table.assign(source_dup = lambda d: d[source]).dropna(subset=source).set_index("source_dup")[target].to_dict()

        return self._concordance_table.dropna(subset=source).set_index(source)[target].to_dict()
    ...
    def get_mapper(self, places, source, target):
        ...
        clean_mapper = {clean_string(k): v for k, v in mapper.items()} # clean the keys for better matching
    ...
    def map(self, places: list[str], source, target, not_found: str = "raise") -> dict[str, str]:
...
class PlaceResolver:
    ...
    def get_mapper(self,
                   places: str | list[str],
                   to: str, place_type: Optional[str] = None,
                   not_found="raise",
                   multiple: Literal["raise", "first", "ignore"] = "raise",
                   custom_mapping: Optional[dict] = None) -> dict[str, str | None]:
        ...
        if not place_type:
            ...
            places = self._disambiguator.resolve(places, not_found=not_found, multiple=multiple)

            if to != "dcid":
                # get the mapping of the places to their Data Commons IDs
                dcids_mapper = self._concordance.get_mapper(places.values(), source="dcid", target=to)
                # map the values in placed to the keys in dcids_mapper
                places = {place: dcids_mapper.get(dcid) for place, dcid in places.items()}
        ...
        # if place_type is provided, get the mapping of the places to the target format
            ...
            mapper = self._concordance.map(places, source=place_type, target=to, not_found=not_found)
    ...
    def convert(self,
        ...
        mapper = self.get_mapper(places, place_type, to, not_found, multiple, custom_mapping)
